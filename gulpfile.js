...
const config = {
    libraryName: 'amexio-ng-extensions',
    unscopedLibraryName: 'amexio-ng-extensions',
    allSrc: 'src/**/*',
    allTs: 'src/**/!(*.spec).ts',
...
const distThemeFolder = path.join(rootFolder, `${config.outputDirTheme}`);
...
const startKarmaServer = (isTddMode, hasCoverage, cb) => {
    ...
    let config = { configFile: `${__dirname}/karma.conf.js`, action: 'watch', showStack: true };
...
gulp.task('build', ['clean'], (cb) => {
    runSequence('compile', 'npm-package', 'rollup-bundle', 'copy-themes', () => {
        process.exit(0)
    });
});
    ...
    return Promise.resolve()
        ...
        .then(() => {
            ...
            const globals = {
                ...
                '@angular/language-service': 'ng.language-services',
...
gulp.task('coveralls', (cb) => {
    pump(
        [
            gulp.src(`${config.coverageDir}/lcov/lcov.info`),
            gulpCoveralls()
        ], cb);
});
